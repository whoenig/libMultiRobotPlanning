      ...
      for (size_t y = 0; y < dimy; ++y) {
        ...
        if (obstacles.find(l) == obstacles.end()) {
          ...
          if (x < dimx - 1 && obstacles.find(right) == obstacles.end()) {
            auto e = boost::add_edge(locToVert(l), locToVert(right), searchGraph);
            searchGraph[e.first].weight = 1;
          }
          Location below(x, y+1);
          if (y < dimy - 1 && obstacles.find(below) == obstacles.end()) {
            auto e = boost::add_edge(locToVert(l), locToVert(below), searchGraph);
            searchGraph[e.first].weight = 1;
          }
        }
      }
...
private:
  ...
  typedef boost::adjacency_list<
          ...
          Vertex, Edge>
          searchGraph_t;
  typedef boost::exterior_vertex_property<searchGraph_t, int> distanceProperty_t;
  ...
  public:
    explicit VertexDotWriter(
      const searchGraph_t& graph,
      size_t dimx)
      : m_graph(graph)
      , m_dimx(dimx)
    {
